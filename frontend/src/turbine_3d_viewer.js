...
class Turbine3DViewer {
    constructor(canvasElement) {
        ...
        this._cx = 0;
        this._cy = 0;
        this._runnerRadius = 0;
        this._hubRadius = 0;
        this._guideRadius = 0;
        this._spiralRadius = 0;
    }

    render(bladeStatuses, selectedBlade, time) {
        ...
        this._cx = cx;
        this._cy = cy;
        this._runnerRadius = runnerRadius;
        this._hubRadius = hubRadius;
        this._guideRadius = guideRadius;
        ...
        drawSensorMarkers(ctx, cx, cy, runnerRadius, spiralRadius);
        drawComponentLabels(ctx, cx, cy, runnerRadius, spiralRadius, height);

        if (selectedBlade && selectedBlade > 0) {
            var rotationOffset = (time || 0) * 0.0002;
            var baseAngle = ((selectedBlade - 1) / BLADE_COUNT) * Math.PI * 2 - Math.PI / 2;
            var angle = baseAngle + rotationOffset;
            var tipX = cx + runnerRadius * 0.85 * Math.cos(angle);
            var tipY = cy + runnerRadius * 0.85 * Math.sin(angle);

            var bladeStatus = null;
            if (bladeStatuses) {
                for (var s = 0; s < bladeStatuses.length; s++) {
                    if (bladeStatuses[s].blade_id === selectedBlade && bladeStatuses[s].zone_id === 1) {
                        bladeStatus = bladeStatuses[s];
                        break;
                    }
                }
            ...
            drawBladeInfo(ctx, tipX, tipY, selectedBlade, bladeStatus, null);
        }
    }

    hitTest(x, y) {
        return hitTestBlade(x, y, this._cx, this._cy, this._runnerRadius, BLADE_COUNT);
    }
}

export { Turbine3DViewer, BLADE_COUNT };
